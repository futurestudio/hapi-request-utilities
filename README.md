...
<p align="center">
    <a href="https://travis-ci.org/fs-opensource/hapi-request-utilities"><img src="https://camo.githubusercontent.com/9f56ef242c6f588f74f39f0bd61c1acd34d853af/68747470733a2f2f7472617669732d63692e6f72672f66732d6f70656e736f757263652f686170692d67656f2d6c6f636174652e7376673f6272616e63683d6d6173746572" alt="Build Status" data-canonical-src="https://travis-ci.org/fs-opensource/hapi-request-utilities.svg?branch=master" style="max-width:100%;"></a>
...
`hapi-request-utilities` is built on top of these solid and awesome projects:

- [async-ratelimiter](https://github.com/microlinkhq/async-ratelimiter)
- [ioredis](https://github.com/luin/ioredis)
- [request-ip](https://github.com/pbojinov/request-ip)

Each package solves its own problem perfectly. `hapi-request-utilities` composes the solutions of each problem to a solid rate limit plugin for hapi.
...
## Usage
The most straight forward to use `hapi-request-utilities` is to register it to your hapi server.
...
- **userLimitKey**: `(string)`, default: `undefined` — define the property name that defines the [dynamic rate limit](https://github.com/fs-opensource/hapi-request-utilities#dynamic-rate-limits). This option is used to access the value from `request.auth.credentials`.
...
await server.register({
  ...
  options: {
    ...
    namespace: 'hapi-request-utilities',
...
## Dynamic Rate Limits
To make use of the dynamic rate limit, you need to configure `hapi-request-utilities` to use the attribute `'rateLimit'`. To do so, configure the `userLimitKey: 'rateLimit'` option during plugin registration.
...
`hapi-request-utilities` uses the default limit if the request is unauthenticated or `request.auth.credentials.rateLimit` is not available.
